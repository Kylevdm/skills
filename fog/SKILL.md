...
3. Read `docs/agents/issue-tracker.md` when present and use its Wayfinding operations. Otherwise infer the tracker from repository evidence; fall back to local Markdown when none is configured.
4. When the ledger does not exist, initialize it from [assets/fog.md](assets/fog.md). A bare invocation then continues as the first sweep.
5. In a read-only repository, return the exact proposed change and location without claiming it was persisted.
...
3. Write a Deferred effort in one to three lines: what it is, where it touches the build with evidence when applicable, and its trigger. Use `Trigger: none yet` rather than forcing one.
...
3. Reject invalid closure states:
   ...
   - A patch explicitly beyond the destination belongs under **Out of scope** and, when still wanted but universally unowned, under ledger **Deferred efforts**—never Carried.
...
5. Stage closure in this order:
   ...
   2. Leave each original patch in **Not yet specified** and mark it inline with its classification, evidence, and rationale.
