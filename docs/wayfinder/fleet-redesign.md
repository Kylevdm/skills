...
- [Specify check-command discovery and the checking stage contract](fleet-redesign/tickets/16-specify-check-command-discovery.md): `checking` is a stage without a model — Fleet runs a human-configured, `fleet-update`-proof command list as its own child and seals the exit codes; no check command means refused admission, the writer's self-report is never evidence, and each of ticket 06's three check points seals its own stage.
